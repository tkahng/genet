...
def network1():
    ...
    n1.add_node('101982', x_y=(528704.1425925883, 182068.78193707118))
    n1.add_node('101986', x_y=(528835.203274008, 182006.27331298392))
...
def network2():
    ...
    n2.add_node('101982', x_y=(51.52287873323954, -0.14625948709424305,))
    n2.add_node('101990', x_y=(51.5205729332399, -0.14770188709624754))
...
def test_adding_networks_with_clashing_node_ids():
    ...
    n_right = Network('epsg:27700')
    n_right.add_node('10', x_y=(528704.1425925883, 182068.78193707118))
    n_right.add_node('20', x_y=(528835.203274008, 182006.27331298392))
...
def test_adding_disjoint_networks_with_unique_ids():
    ...
    n_right = Network('epsg:27700')
    n_right.add_node('10', x_y=(528701, 182070))
    n_right.add_node('20', x_y=(528837, 182008))
    n_right.add_link('100', '10', '20', attribs={'modes': ['walk']})

    n_left.add(n_right)
    assert_semantically_equal(dict(n_left.nodes()),
                              {'10': {'id': '10', 'x': 528701, 'y': 182070, 'lat': 51.52289039507891,
                                      'lon': -0.1463043142064796, 's2_id': 5221390329378786147},
                               '20': {'id': '20', 'x': 528837, 'y': 182008, 'lat': 51.52230224101315,
                                      'lon': -0.14436777335634526, 's2_id': 5221390328605636423},
...
def test_adding_disjoint_networks_with_clashing_ids():
    ...
    n_right = Network('epsg:27700')
    n_right.add_node('1', x_y=(1, 1))
    n_right.add_node('2', x_y=(1, 2))
    n_right.add_link('1', '1', '2', attribs={'modes': ['walk']})
...
def test_node_attribute_data_under_key_returns_correct_pd_series_with_nested_keys():
    n = Network('epsg:27700')
    n.add_node(1, x_y=(1, 1), attribs={'a': {'b': 1}})
    n.add_node(2, x_y=(2, 2), attribs={'a': {'b': 4}})
...
def test_node_attribute_data_under_key_returns_correct_pd_series_with_flat_keys():
    n = Network('epsg:27700')
    n.add_node(1, x_y=(1, 1), attribs={'b': 1})
    n.add_node(2, x_y=(2, 2), attribs={'b': 4})
...
def test_node_attribute_data_under_keys_generates_key_for_nested_data(network1):
    network1.add_node('1', x_y=(1, 1), attribs={'key': {'nested_value': {'more_nested': 4}}})
...
def test_add_node_adds_node_to_graph_with_attribs():
    n = Network('epsg:27700')
    n.add_node(1, x_y=(1, 1), attribs={'a': 1})
    assert n.graph.has_node(1)
    assert_semantically_equal(n.node(1),
                              {'a': 1, 'id': 1, 'x': 1, 'y': 1, 'lat': 49.76681685192715, 'lon': -7.557147012910978,
                               's2_id': 5205973754090381381})


def test_add_node_adds_node_to_graph_without_additional_attribs():
    n = Network('epsg:27700')
    n.add_node(1, x_y=(1, 1))
    assert n.graph.has_node(1)
    assert_semantically_equal(n.node(1),
                              {'id': 1, 'x': 1, 'y': 1, 'lat': 49.76681685192715, 'lon': -7.557147012910978,
                               's2_id': 5205973754090381381})
    ...
    correct_change_log_df = pd.DataFrame(
        {'change_event': {3: 'modify', 4: 'modify', 5: 'modify'}, 'object_type': {3: 'link', 4: 'node', 5: 'node'},
         'old_id': {3: '0', 4: '101982', 5: '101982'}, 'new_id': {3: '0', 4: '007', 5: '101982'}, 'old_attributes': {
            ...
            4: "{'id': '101982', 'x': 528704.1425925883, 'y': 182068.78193707118, 'lat': 51.52287873323954, 'lon': -0.14625948709424305, 's2_id': 5221390329378179879}",
            5: "{'id': '101982', 'x': 528704.1425925883, 'y': 182068.78193707118, 'lat': 51.52287873323954, 'lon': -0.14625948709424305, 's2_id': 5221390329378179879}"},
         'new_attributes': {
             3: "{'id': '0', 'from': '007', 'to': '101986', 'freespeed': 4.166666666666667, 'capacity': 600.0, 'permlanes': 1.0, 'oneway': '1', 'modes': ['car'], 's2_from': 5221390329378179879, 's2_to': 5221390328605860387, 'length': 52.765151087870265, 'attributes': {'osm:way:access': {'name': 'osm:way:access', 'class': 'java.lang.String', 'text': 'permissive'}, 'osm:way:highway': {'name': 'osm:way:highway', 'class': 'java.lang.String', 'text': 'unclassified'}, 'osm:way:id': {'name': 'osm:way:id', 'class': 'java.lang.Long', 'text': '26997928'}, 'osm:way:name': {'name': 'osm:way:name', 'class': 'java.lang.String', 'text': 'Brunswick Place'}}}",
             4: "{'id': '007', 'x': 528704.1425925883, 'y': 182068.78193707118, 'lat': 51.52287873323954, 'lon': -0.14625948709424305, 's2_id': 5221390329378179879}",
             5: "{'id': '007', 'x': 528704.1425925883, 'y': 182068.78193707118, 'lat': 51.52287873323954, 'lon': -0.14625948709424305, 's2_id': 5221390329378179879}"},
...
def test_modify_node_adds_attributes_in_the_graph_and_change_is_recorded_by_change_log():
    ...
    assert 'b' in n.node(1)
    assert n.node(1)['b'] == 1
    assert 'a' in n.node(1)
    assert n.node(1)['a'] == 1

    correct_change_log_df = pd.DataFrame(
        {'timestamp': {0: '2020-07-25 21:12:12', 1: '2020-07-25 21:12:12'}, 'change_event': {0: 'add', 1: 'modify'},
         'object_type': {0: 'node', 1: 'node'}, 'old_id': {0: None, 1: 1}, 'new_id': {0: 1, 1: 1},
         'old_attributes': {0: None,
                            1: "{'a': 1, 'id': 1, 'x': 1, 'y': 1, 'lat': 49.76681685192715, 'lon': -7.557147012910978, 's2_id': 5205973754090381381}"},
         'new_attributes': {
             0: "{'a': 1, 'id': 1, 'x': 1, 'y': 1, 'lat': 49.76681685192715, 'lon': -7.557147012910978, 's2_id': 5205973754090381381}",
             1: "{'a': 1, 'id': 1, 'x': 1, 'y': 1, 'lat': 49.76681685192715, 'lon': -7.557147012910978, 's2_id': 5205973754090381381, 'b': 1}"},
         'diff': {0: [('add', '', [('a', 1), ('id', 1), ('x', 1), ('y', 1), ('lat', 49.76681685192715),
                                   ('lon', -7.557147012910978), ('s2_id', 5205973754090381381)]), ('add', 'id', 1)],
                  1: [('add', '', [('b', 1)])]}})
...
def test_modify_node_overwrites_existing_attributes_in_the_graph_and_change_is_recorded_by_change_log():
    ...
    correct_change_log_df = pd.DataFrame(
        {'change_event': {0: 'add', 1: 'modify'}, 'object_type': {0: 'node', 1: 'node'}, 'old_id': {0: None, 1: 1},
         'new_id': {0: 1, 1: 1}, 'old_attributes': {0: None,
                                                    1: "{'a': 1, 'id': 1, 'x': 1, 'y': 1, 'lat': 49.76681685192715, 'lon': -7.557147012910978, 's2_id': 5205973754090381381}"},
         'new_attributes': {
             0: "{'a': 1, 'id': 1, 'x': 1, 'y': 1, 'lat': 49.76681685192715, 'lon': -7.557147012910978, 's2_id': 5205973754090381381}",
             1: "{'a': 4, 'id': 1, 'x': 1, 'y': 1, 'lat': 49.76681685192715, 'lon': -7.557147012910978, 's2_id': 5205973754090381381}"},
         'diff': {0: [('add', '', [('a', 1), ('id', 1), ('x', 1), ('y', 1), ('lat', 49.76681685192715),
                                   ('lon', -7.557147012910978), ('s2_id', 5205973754090381381)]), ('add', 'id', 1)],
                  1: [('change', 'a', (1, 4))]}})
...
def test_modify_nodes_adds_and_changes_attributes_in_the_graph_and_change_is_recorded_by_change_log():
    n = Network('epsg:27700')
    n.add_node(1, x_y=(1, 1), attribs={'a': 1})
    n.add_node(2, x_y=(1, 1), attribs={'b': 1})
    n.apply_attributes_to_nodes({1: {'a': 4}, 2: {'a': 1}})

    assert_semantically_equal(n.node(1),
                              {'a': 4, 'id': 1, 'x': 1, 'y': 1, 'lat': 49.76681685192715, 'lon': -7.557147012910978,
                               's2_id': 5205973754090381381})
    assert_semantically_equal(n.node(2),
                              {'b': 1, 'id': 2, 'x': 1, 'y': 1, 'lat': 49.76681685192715, 'lon': -7.557147012910978,
                               's2_id': 5205973754090381381, 'a': 1})

    correct_change_log_df = pd.DataFrame(
        {'timestamp': {0: '2020-07-25 21:16:15', 1: '2020-07-25 21:16:15', 2: '2020-07-25 21:16:15',
                       3: '2020-07-25 21:16:15'}, 'change_event': {0: 'add', 1: 'add', 2: 'modify', 3: 'modify'},
         'object_type': {0: 'node', 1: 'node', 2: 'node', 3: 'node'}, 'old_id': {0: None, 1: None, 2: 1, 3: 2},
         'new_id': {0: 1, 1: 2, 2: 1, 3: 2}, 'old_attributes': {0: None, 1: None,
                                                                2: "{'a': 1, 'id': 1, 'x': 1, 'y': 1, 'lat': 49.76681685192715, 'lon': -7.557147012910978, 's2_id': 5205973754090381381}",
                                                                3: "{'b': 1, 'id': 2, 'x': 1, 'y': 1, 'lat': 49.76681685192715, 'lon': -7.557147012910978, 's2_id': 5205973754090381381}"},
         'new_attributes': {
             0: "{'a': 1, 'id': 1, 'x': 1, 'y': 1, 'lat': 49.76681685192715, 'lon': -7.557147012910978, 's2_id': 5205973754090381381}",
             1: "{'b': 1, 'id': 2, 'x': 1, 'y': 1, 'lat': 49.76681685192715, 'lon': -7.557147012910978, 's2_id': 5205973754090381381}",
             2: "{'a': 4, 'id': 1, 'x': 1, 'y': 1, 'lat': 49.76681685192715, 'lon': -7.557147012910978, 's2_id': 5205973754090381381}",
             3: "{'b': 1, 'id': 2, 'x': 1, 'y': 1, 'lat': 49.76681685192715, 'lon': -7.557147012910978, 's2_id': 5205973754090381381, 'a': 1}"},
         'diff': {0: [('add', '', [('a', 1), ('id', 1), ('x', 1), ('y', 1), ('lat', 49.76681685192715),
                                   ('lon', -7.557147012910978), ('s2_id', 5205973754090381381)]), ('add', 'id', 1)],
                  1: [('add', '', [('b', 1), ('id', 2), ('x', 1), ('y', 1), ('lat', 49.76681685192715),
                                   ('lon', -7.557147012910978), ('s2_id', 5205973754090381381)]), ('add', 'id', 2)],
...
def test_apply_function_to_nodes():
    n = Network('epsg:27700')
    n.add_node('0', x_y=(1, 1), attribs={'a': 2, 'c': 3})
    n.add_node('1', x_y=(1, 1), attribs={'c': 100})
    ...
    assert_semantically_equal(dict(n.nodes()),
                              {'0': {'a': 2, 'c': 3, 'id': '0', 'x': 1, 'y': 1, 'lat': 49.76681685192715,
                                     'lon': -7.557147012910978, 's2_id': 5205973754090381381, 'new_computed_attrib': 6},
                               '1': {'c': 100, 'id': '1', 'x': 1, 'y': 1, 'lat': 49.76681685192715,
                                     'lon': -7.557147012910978, 's2_id': 5205973754090381381}})
...
def test_retaining_n_connected_subgraphs():
    n = Network('epsg:27700')
...
def test_read_matsim_network_delegates_to_matsim_reader_read_network(mocker):
    ...
    matsim_reader.read_network.assert_called_once_with(pt2matsim_network_test_file, network.transformer_to_lat_lon)
...
def test_has_nodes_when_nodes_in_the_graph():
    n = Network('epsg:27700')
    n.add_node('1', x_y=(1, 1))
    n.add_node('2', x_y=(1, 1))
    n.add_node('3', x_y=(1, 1))
...
def test_calculate_route_distance_with_links_that_dont_have_length_attrib():
    n = Network('epsg:27700')
    n.add_node(1, x_y=(1, 1))
    n.add_node(3, x_y=(1, 2))
    n.add_node(4, x_y=(1, 3))
    n.add_link('1', 1, 3)
    n.add_link('2', 3, 4)
